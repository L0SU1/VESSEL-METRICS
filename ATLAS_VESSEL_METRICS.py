...
def compute_metrics_for_mask(mask, dist_map, affine, header, selected_metrics, save_segment_masks,save_conn_comp_masks):
    ...
    '''
    if 'betti_numbers' in selected_metrics:
        b0, b1, b2 = compute_betti_numbers(mask, G)
        results['betti_0'] = b0
        results['betti_1'] = b1
        results['betti_2'] = b2
    '''
...
def process(mask_path, atlas_path, selected_metrics, save_segment_masks,save_conn_comp_masks):
    """
    Updated process function using SimpleITK registration
    """
    # Load binary mask using nibabel for distance map
    img = nib.load(mask_path)
    arr = img.get_fdata() > 0
    affine, header = img.affine, img.header
    
    # Distance map for metric calculations
    dist_map = distance_transform_edt(arr)
    
    # Load mask and atlas as SimpleITK images
    fixed_img = sitk.ReadImage(mask_path)
    moving_img = sitk.ReadImage(atlas_path)
    
    # Convert both images to float32 for registration (required by SimpleITK)
    ...
    # Set metric (Mutual Information is robust for different image types)
    ...
    registered_atlas = sitk.Resample(
        ...
        fixed_img,
        final_transform,
        sitk.sitkNearestNeighbor,  # Use nearest neighbor for label images
        0.0,  # Default pixel value
        moving_img.GetPixelID()
    )
    
    # Convert registered atlas to numpy and reorient from (z, y, x) → (x, y, z)
    atlas = np.transpose(sitk.GetArrayFromImage(registered_atlas).astype(int), (2, 1, 0))
    ...
    for label in np.unique(atlas):
        ...
        region_result = compute_metrics_for_mask(
            region_mask, dist_map, affine, header, selected_metrics, save_segment_masks, save_conn_comp_masks
        )
